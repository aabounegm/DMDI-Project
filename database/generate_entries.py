...
# BS18-04: Abdelrahman Abounegm, Georgiy Stepanov, Vitaliy Korbashov, Pavel Tishkin, Yulia Chukanova.
...
#generates either "true" or "false" string:
def generateBool():
    if(randint(0,1)):
        return 'true'
    return 'false'

#generates Cost of a Doctor per hour in string:
def generateCost():
    return str(randint(1, 70) * 100)

#generates Nurse's monthly salary in string:
def generateNSalary():
    return str(randint(100, 500) * 100)

#generates Staff' monthly salary in string:
def generateSSalary():
    return str(randint(25, 5240) * 100)

#generates unique license_id in string:
LicenseIDs=set()
...
#generates unique phone_number in string surrounded by single quotes (format: '+79...'):
phnumbers=set()
...
#generates (non-unique) room in string surrounded by single quotes:
def generateRoom():
    room=str(randint(1,5)*100 + randint(1,99))
    t=randint(0,10)
    if(t==1 or t==2):    #note that either 'A', 'B', 'C' or nothing can be added in the end of the three-digit number:
...
#generates 1 of 6 possible blood types, surrounded by single quotes:
bloodTypes=['A','B','AB','O']
def generateBlood():
    t=randint(0,3)
    if randint(0,1):
...
#generates Patients' emergency_contact_name (which is 'Name Surname'),
#in string surrounded by single quotes:
def generateECName():
    return choice(firstnames)[0:-1] + ' ' + choice(lastnames)[1:]

#generates the number of Syndicate to which the Person belongs to:
def generateNumSyndicate():
    if(randint(0,3)):
        return str(randint(1, numSyndicates))
    return 'null'

#generates Patient's date_of_birth in format 'yyyy.mm.dd' in string surrounded by single quotes:
def generateDOB():
    ...
    if(len(day)<2):
        day='0'+day
    return singleQuote + str(randint(1950,2015)) + '.' + month + '.' + day + singleQuote

#generates a string from "00" to "55" divisible by 5:
def generateMinute():
...
#generates date and time for Appointment in isoformat using datetime.isoformat():
def generateDateTime():
    return datetime(randint(2000,2019),randint(1,12),randint(1,28),randint(7,19),int(generateMinute())).isoformat()

#generates the time from which a person starts working in the morning (from '07:00' to '10:55'),
#in string surrounded by single quotes:
def generateTime1():
...
#generates the time from which a person has lunch-break (from '12:00' to '13:55'),
#in string surrounded by single quotes:
def generateTime2():
    return singleQuote + str(randint(12,13)) + ':' + generateMinute() + singleQuote

#generates the time from which a person starts working in the evening (till which has lunch-break),
#that is from '15:00' to '16:55', in string surrounded by single quotes:
def generateTime3():
    return singleQuote + str(randint(15,16)) + ':' + generateMinute() + singleQuote

#generates the time till which a person works in the evening (the end of a workday),
#that is from '18:00' to '22:55', in string surrounded by single quotes:
def generateTime4():
...
for i in range(numDoctors):         #generate INSERT statements for Doctors:
    fout.write('INSERT INTO Doctors '+
               '(first_name, last_name, license_id, speciality, cost, room, phone_number) VALUES (' +
               choice(firstnames) + comma + choice(lastnames) + comma + generateLicenseID() +
               comma + choice(specialists) + comma + generateCost() + comma + generateRoom() +
               comma + generatePhnumber() + ');\n')
fout.write('\n')
for doc in range(1, numDoctors+1):         #generate Working hours for each Doctor...
    for day in range(1, 6):                     #...on each day:
        if(randint(0,1)):
            #Working hours before lunch-break:
            fout.write('INSERT INTO Doctor_Working_Hours (doctor_id, start_time, end_time, day) VALUES ('+
                       str(doc) + comma + generateTime1() + comma + generateTime2() + comma + str(day)+ ');\n')
            #Working hours after lunch-break
            fout.write('INSERT INTO Doctor_Working_Hours (doctor_id, start_time, end_time, day) VALUES ('+
                       str(doc) + comma + generateTime3() + comma + generateTime4() + comma + str(day) + ');\n')
fout.write('\n')
...
for i in range(numNurses):         #generate INSERT statements for Nurses:
    fout.write('INSERT INTO Nurses (first_name, last_name, license_id, phone_number, salary) VALUES ('+
               choice(firstnames) + comma + choice(lastnames) + comma + generateLicenseID() +
               comma + generatePhnumber() + comma +generateNSalary() + ');\n')
fout.write('\n')
for nurse in range(1, numNurses+1):       #generate Working hours for each Nurse...
    for day in range(1, 6):                     #...on each day:
        if(randint(0,1)):
            #Working hours before lunch-break:
            fout.write('INSERT INTO Nurse_Working_Hours (nurse_id, start_time, end_time, day) VALUES ('+
...
for i in range(numStaff):           #generate INSERT statements for Staff:
    fout.write('INSERT INTO Staff (first_name, last_name, job, phone_number, salary) VALUES ('+
               choice(firstnames) + comma + choice(lastnames) + comma + choice(jobs) +
               comma + generatePhnumber() + comma + generateSSalary() + ');\n')
fout.write('\n')
for staff in range(1, numStaff+1):       #generate Working hours for each Staff...
    for day in range(1, 6):                     #...on each day:
        if(randint(0,1)):
            #Working hours before lunch-break:
            fout.write('INSERT INTO Staff_Working_Hours (staff_id, start_time, end_time, day) VALUES ('+
...
if(numSyndicates<0 or numSyndicates>30):
    ...
    numSyndicates=30
if(numSyndicates!=0):
    with open('Syndicates.txt','r') as fsyndicates:     #generate atomic INSERT statement for Syndicates:
...
for i in range(numPatients):            #generate INSERT statements for Patients:
    fout.write('INSERT INTO Patients (first_name, last_name, date_of_birth, blood_type, ' +
               'phone_number, syndicate_id, emergency_contact_name, emergency_contact_relation, ' +
               'emergency_contact_phone_number) VALUES (' + choice(firstnames) + comma +
               choice(lastnames) + comma + generateDOB() + comma + generateBlood() + comma +
               generatePhnumber() + comma + generateNumSyndicate() + comma + generateECName() +
               comma + choice(relations) + comma + generatePhnumber() + ');\n')
fout.write('\n')
...
for i in range(numAppointments):         #generate INSERT statements for Appointmens:
    fout.write("INSERT INTO Appointments (date, doctor_id, patient_id, ailment_description) VALUES ('"+
               generateDateTime() + singleQuote + comma + str(randint(1,numDoctors)) + comma +
               str(randint(1,numPatients)) + comma + singleQuote + loremIpsum + "');\n")
fout.write('\n')
print(numAppointments, 'Appointments between Doctor and Patient have been successfully created.\n')
...
for i in range(numReports):         #generate INSERT statements for Reports:
    fout.write('INSERT INTO Reports '+
...
fout.write('\n')
print(numReports, 'Reports belonging to Appointments have been successfully created.\n')
...
if(numNoticeBoards<0):
    print('Using the default value of 100...')
    numNoticeBoards=100
for i in range(numNoticeBoards):         #generate INSERT statements for NoticeBoards:
    fout.write("INSERT INTO Notice_Boards (name, description, creator_id) VALUES ('Notice Board Name "+
...
for i in range(numNotices):         #generate INSERT statements for Notices:
    fout.write('INSERT INTO Notices (board_id, title, content, date_posted, doctor_id, nurse_id) VALUES (' +
               str(randint(1,numNoticeBoards)) + comma + "'Notice " + str(i+1) + " title'" + comma + singleQuote +
               loremIpsum + singleQuote + comma + singleQuote + generateDateTime() + singleQuote + comma)
    if(randint(0,2)):
...
for i in range(numDS):         #generate INSERT statements for Doctor_Subscription:
    fout.write('INSERT INTO Doctor_Subscription (doctor_id, board_id) VALUES (' +
               str(randint(1,numDoctors)) +comma + str(randint(1,numNoticeBoards)) +');\n')
fout.write('\n')
...
for i in range(numNS):         #generate INSERT statements for Nurse_Subscription:
    fout.write('INSERT INTO Nurse_Subscription (nurse_id, board_id) VALUES (' +
               str(randint(1,numNurses)) +comma + str(randint(1,numNoticeBoards)) +');\n')
fout.write('\n')
...
for i in range(numPS):         #generate INSERT statements for Patient_Subscription:
    fout.write('INSERT INTO Patient_Subscription (patient_id, board_id) VALUES (' +
               str(randint(1,numPatients)) +comma + str(randint(1,numNoticeBoards)) +');\n')
fout.write('\n')
